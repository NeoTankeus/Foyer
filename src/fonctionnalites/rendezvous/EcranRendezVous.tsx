...
export function EcranRendezVous() {
  ...
  const demanderIdees = async () => {
    ...
    try {
      ...
      setIdees(donnees.reponse ?? donnees.message ?? 'STG n’a pas répondu — réessaie.')
    } catch {
  ...
  return (
    <div className="px-5 pb-6 pt-3">
      ...
      <Carte>
        ...
        <div className="mt-3">
          <Bouton pleineLargeur variante="primaire" desactive={reflechit} onClick={() => void demanderIdees()}>
            {reflechit ? 'STG réfléchit…' : '✨ Demander 3 idées à STG'}
          </Bouton>
        </div>
      </Carte>
      ...
      {idees && (
        <Carte>
          <h3 className="mb-1 text-note font-[590] uppercase tracking-wide text-encre-3">Les idées de STG</h3>
